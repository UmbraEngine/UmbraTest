...
static void BeginContext(TestRunner* runner, const char* groupName, const char* testName)
{
    runner->currentGroup = groupName;
    runner->currentTest = testName;
    runner->inTest = true;
}

static void EndContext(TestRunner* runner)
{
    runner->inTest = false;
}
...
void TestRunner::TestFail(const char* file, int line, const char* fmt, ...)
{
    ...
    std::fprintf(
        stderr, "%s:%d: FAIL [%s] %s: ", file, line, 
        CheckGroupName(g_runner->currentGroup),
...
)
{
    ...
    if (group->GetBeforeEach()) {
        BeginContext(this, group->GetName(), "(before_each)");
        group->ExecuteBeforeEach();
        EndContext(this);
    }

    BeginContext(this, group->GetName(), test->GetName());
    test->Run();
    EndContext(this);

    if (group->GetAfterEach()) {
        BeginContext(this, group->GetName(), "(after_each)");
    ...
    if (this->isFailure) {
        ...
        summary->passed += 1;
        TEST_LOG_PRINT(LogColor::Green, "PASS [%s] %s", CheckGroupName(group->GetName()), test->GetName());
    }
}

static TestRunSummary* RunGroup(TestRunner* runner, TestGroup* group, TestRunSummary* summary)
{
    if (!group) {
        return summary;
    }
...
#ifdef ENABLE_DEBUG
    TEST_LOG_PRINT(
        LogColor::Yellow, "[%s] - Test Count: %zu", 
        group->GetName(), group->GetTests().size()
    );
#else
    ...
    if (group->GetBeforeAll()) {
        BeginContext(runner, group->GetName(), "(before_all)");
    ...
    for (size_t i = 0; i < tests.size(); ++i) {
        runner->RunOne(group, &tests[i], summary);
    }

    for (size_t i = 0; i < group->GetChildCount(); ++i) {
        RunGroup(runner, group->GetChild(i), summary);
        std::printf("\n");
    }

    if (group->GetAfterAll()) {
        BeginContext(runner, group->GetName(), "(after_all)");
...
TestRunSummary TestRunner::RunAll(const TestRegistry* registry)
{
    ...
    RunGroup(this, registry->GetRootGroup(), &summary);
